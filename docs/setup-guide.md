...
Windsurf/Devin Desktop (Cascade)
  │
  ├── Cascade Hooks (~/.codeium/windsurf/hooks.json)
  │     │
  │     └── cascade-bridge.mjs (~/.agentmemory/scripts/)
  │           │
  ...
  └── Global Rule (~/.codeium/windsurf/memories/global_rules.md)
        │
        └── Instructs agent to auto-search memory at session start
```
...
Add the agentmemory MCP server to Devin's MCP config.

### Windows

Edit `%APPDATA%\devin\mcp_config.json` and add:
...
> The MCP client (`@agentmemory/mcp`) is a local stdio process that proxies all tool calls to the remote daemon at `AGENTMEMORY_URL` using `AGENTMEMORY_SECRET` for HMAC auth. Alternatively, if your daemon exposes a public `/mcp` HTTP endpoint, you can use the `url` + `headers` form instead:
> ```json
...
Edit `~/.config/devin/mcp_config.json` with the same structure.
